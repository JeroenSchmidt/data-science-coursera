...
$$
***Aside Notation***:  $θ^Tx=z$  

**1)** To make a support vector machine, we will modify the first term of the cost function $-\log(h_{\theta}(x)) = -\log\Big(\dfrac{1}{1 + e^{-\theta^Tx}}\Big)$.

- $z>1 \text{ then } -log(h_\theta(x))=0$. 
- $z<1$ , we shall use a **straight decreasing line** instead of the sigmoid curve.

(In the literature, this is called a **hinge loss** ([https://en.wikipedia.org/wiki/Hinge_loss)](https://en.wikipedia.org/wiki/Hinge_loss)) function.)
...
**2)** Similarly, we modify the second term of the cost function $−\log(1−h_θ(x))=−\log(1−\frac{1}{1+e−θ^Tx})$.
...
$Θ^Tx≥1$ if y=1 and $Θ^Tx≤−1$ if y=0.
...
$$
We can use the same rules to rewrite $Θ^Tx^{(i)}$:
$$
\Theta^Tx^{(i)} = p^{(i)} \cdot ||\Theta || = \Theta_1x_1^{(i)} + \Theta_2x_2^{(i)} + \dots + \Theta_n x_n^{(i)}
$$
So we now have a new **optimization objective** by substituting $p^{(i)}⋅||Θ||$ in for $Θ^Tx^{(i)}$:
